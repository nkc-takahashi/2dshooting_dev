    ...
    public class StateSort : MonoBehaviour
    {
        ...
        void Bar()
        {
            if (limiterbreak)
            {
                bar.GetComponent<RectTransform>().sizeDelta =
                    new Vector2(sizedelta_width / 2 + (value * sizedelta_width * 2), sizedelta_height);
            }
            else
            {
                bar.GetComponent<RectTransform>().sizeDelta =
                    new Vector2(sizedelta_width + (value * 10), sizedelta_height);
                if (maxvalue <= value)
                {
        ...
        public void OnClickPlus()
        {
            ...
            if (statemanager.GetComponent<StateManager>().Point > 0)
            {
                ...
                {
                    value += 1;
                    statemanager.GetComponent<StateManager>().Point -= 1;
                }
                else
                {
